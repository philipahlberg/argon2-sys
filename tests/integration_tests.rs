...
#[test]
fn case_1() {
    let ty = Argon2_type_Argon2_i;
    let version = Argon2_version_ARGON2_VERSION_10;
    let output = run_hash(ty, version, 2, 16, 1, "password", "somesalt");
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    assert_eq!(
        output.hash,
        hex::decode("f6c4db4a54e2a370627aff3db6176b94a2a209a62c8e36152711802f7b30c694").unwrap(),
    );
}
...
fn case_2() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("3e689aaa3d28a77cf2bc72a51ac53166761751182f1ee292e3f677a7da4c2467").unwrap(),
    );
}
...
fn case_3() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("fd4dd83d762c49bdeaf57c47bdcd0c2f1babf863fdeb490df63ede9975fccf06").unwrap(),
    );
}
...
fn case_4() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("b6c11560a6a9d61eac706b79a2f97d68b4463aa3ad87e00c07e2b01e90c564fb").unwrap(),
    );
}
...
fn case_5() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("81630552b8f3b1f48cdb1992c4c678643d490b2b5eb4ff6c4b3438b5621724b2").unwrap(),
    );
}
...
fn case_6() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("f212f01615e6eb5d74734dc3ef40ade2d51d052468d8c69440a3a1f2c1c2847b").unwrap(),
    );
}
...
fn case_7() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("e9c902074b6754531a3a0be519e5baf404b30ce69b3f01ac3bf21229960109a3").unwrap(),
    );
}
...
fn case_8() {
    ...
    assert_eq!(
        output.hash,
        hex::decode("79a103b90fe8aef8570cb31fc8b22259778916f8336b7bdac3892569d4f1c497").unwrap(),
    );
}
...
fn case_9() {
    ...
    let encoded = "$argon2i$m=65536,t=2,p=1c29tZXNhbHQ$9sTbSlTio3Biev89thdrlKKiCaYsjjYVJxGAL3swxpQ";
...
fn case_10() {
    let ty = Argon2_type_Argon2_i;
    let password = "password";
    let encoded = "$argon2i$m=65536,t=2,p=1$c29tZXNhbHQ9sTbSlTio3Biev89thdrlKKiCaYsjjYVJxGAL3swxpQ";
    let ret = run_verify(ty, encoded, password);
    assert_eq!(ret, Argon2_ErrorCodes_ARGON2_DECODING_FAIL);
}
...
// Tests for version 13 (19)

#[test]
fn case_14() {
    let ty = Argon2_type_Argon2_i;
    let version = Argon2_version_ARGON2_VERSION_13;
    let output = run_hash(ty, version, 2, 16, 1, "password", "somesalt");
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("c1628832147d9720c5bd1cfd61367078729f6dfb6f8fea9ff98158e0d7816ed0").unwrap();
...
fn case_15() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("296dbae80b807cdceaad44ae741b506f14db0959267b183b118f9b24229bc7cb").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_16() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("89e9029f4637b295beb027056a7336c414fadd43f6b208645281cb214a56452f").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_17() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("4ff5ce2769a1d7f4c8a491df09d41a9fbe90e5eb02155a13e4c01e20cd4eab61").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_18() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("d168075c4d985e13ebeae560cf8b94c3b5d8a16c51916b6f4ac2da3ac11bbecf").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_19() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("aaa953d58af3706ce3df1aefd4a64a84e31d7f54175231f1285259f88174ce5b").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_20() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("14ae8da01afea8700c2358dcef7c5358d9021282bd88663a4562f59fb74d22ee").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_21() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("b0357cccfbef91f3860b0dba447b2348cbefecadaf990abfe9cc40726c521271").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
#[test]
fn case_26() {
    let ty = Argon2_type_Argon2_id;
    let version = Argon2_version_ARGON2_VERSION_13;
    let output = run_hash(ty, version, 2, 16, 1, "password", "somesalt");
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("09316115d5cf24ed5a15a31a3ba326e5cf32edc24702987c02b6566f61913cf7").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_27() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("78fe1ec91fb3aa5657d72e710854e4c3d9b9198c742f9616c2f085bed95b2e8c").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_28() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("9dfeb910e80bad0311fee20f9c0e2b12c17987b4cac90c2ef54d5b3021c68bfe").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_29() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("6d093c501fd5999645e0ea3bf620d7b8be7fd2db59c20d9fff9539da2bf57037").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_30() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("f6a5adc1ba723dddef9b5ac1d464e180fcd9dffc9d1cbf76cca2fed795d9ca98").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_31() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("9025d48e68ef7395cca9079da4c4ec3affb3c8911fe4f86d1a2520856f63172c").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_32() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("0b84d652cf6b0c4beaef0dfe278ba6a80df6696281d7e0d2891b817d8c458fde").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
...
fn case_33() {
    ...
    assert_eq!(output.code, Argon2_ErrorCodes_ARGON2_OK);
    let hash =
        hex::decode("bdf32b05ccc42eb15d58fd19b1f856b113da1e9a5874fdcc544308565aa8141c").unwrap();
    assert_eq!(output.hash, hash);
    let encoded =
